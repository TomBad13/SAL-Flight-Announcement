...
try:
    FlightNumber = int(input("How much Flights do you want to announce?  "))
except ValueError:
    print("ValueError: Make sure to specify an Integer.")
    sys.exit(1)
if FlightNumber > 1:
    AutoCallsign = bool(True) if input("Do you want to automatically give next Flights Callsign? [Y/N]  ").upper() == "Y" else bool(False)
...
for i in range(FlightNumber):
    ...
    Flight_Date = input("Flight Date [DD/MM]:  ")
    ...
    message = StaffAnnounce.format(Callsign=Callsign,FH=FH,Airport=Airport,Gate=Gate,Destination=Destination,Flight_Date=Flight_Date,Check_In=Check_In,Briefing_Time=Briefing_Time,Additional_Info=Additional_Info)
